...
export default createStore({
  plugins: [
    vuexLocal.plugin,
    store => store.state.expertIds = [...expertMapIds, ...expertTaskIds],
  ],
  state: {
    loading: true,
    language: null,
  ...
  actions: {
    ...
    save: async ({ dispatch, commit, getters }, { swapId, type })  => {
      ...
      if (swapId && !getters.isExpert(swapId)) commit('delete', swapId)
      return newId
    },
    ...
    loadTask: async ({ dispatch, commit }, id) => {
      // const t = slug => getters.translate(slug)
      const t = slug => slug
      // verify it loads and is a task
      ...
      try {
        ...
        return Promise.reject(new Error( t('id-not-found-or-not-of-valid-type') ))
      }
    },
